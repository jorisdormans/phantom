package nl.jorisdormans.phantom2D.objects 
{
	...
	import nl.jorisdormans.phantom2D.lightEffects.ILightAppearance;
	...
	public class ObjectLayer extends Layer implements IInputHandler
	{
		...
		override public function updatePhysics(elapsedTime:Number):void {
			CollisionData.doIsometric = (this is IsoObjectLayer);
		...
		public function checkCollisionsBetween(object1:GameObject, object2:GameObject):void {
			...
			gameScreen.game.prof.end("collision check");
		}
		...
		override public function handleMessage(message:String, data:Object = null):int 
		{
			switch (message) {
				case "lightChanged":
					for (var i:int = 0; i < objects.length; i++) {
						if (objects[i] is ILightAppearance) {
							(objects[i] as ILightAppearance).changeLight(data.light);
						}
					}
