	...
	public class Screen extends Layer
	{
		...
		public function activate():void {
			...
			for (var i:int = 0; i < components.length; i++) {
				if (components[i] is LightEffectLayer) {
					sendMessage("lightChanged", { light: (components[i] as LightEffectLayer).light } );
				}
			}
		}
		...
		public function addLightSource(x:Number, y:Number, radius:Number, strength:Number=1, color:uint=0xffffff):void {
			...
			for (var i:int = l-1; i >= 0 ; i--) {
				if (components[i] is LightEffectLayer) {
					(components[i] as LightEffectLayer).addLightSource(x, y, radius, strength, color);
					return;
				}
			}
		}
		...
		public function addPlayerLight(x:Number, y:Number, radius:Number, strength:Number = 0.25):void {
			...
			for (var i:int = l-1; i >= 0 ; i--) {
				if (components[i] is LightEffectLayer) {
					(components[i] as LightEffectLayer).addPlayerLight(x, y, radius, strength);
					return;
				}
			}
		}
